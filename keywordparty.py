# -*- coding: utf8 -*-
# ॐ  keywordparty (El Aye, List) [List Against LIst X-Stractor Reveal Leftovers COOL 2 Lists Make 3]
import argparse # important import :)
parser = argparse.ArgumentParser(description='\"keywordparty\" El-Aye-List by Shaun R. Best KG4NWV CopyFree 2025\r\n\r\nExample [  C:\\Users\\You> python keywordparty.py -n text.file.1 textfile2.txt↵  ]\r\n\r\nPlease see lalist on Github. keywordparty is the same program for Windows / Dos users and lalist is the Linux users version. Maybe you make lists of keywords for SEO Search Engine Optimization or some other reason you need to work with lists of one word which may also contain the normal 32 specail characters and have the power to compare lists and generate inverse lists of keywords which are not in the other list and generate a list of keywords matching in both lists. The input files are not written to and 3 files will be output and all duplicates will be removed. Character case is sensitive as in \'a\' does not equal \'A\'.\r\nThe OUTPUT of the program will be 3 files.\r\ncommon_match (A new list of words which appear in both lists.)\r\nleftover1 (Words from 1st list which didn\'t match the second list.)\r\nleftover2 (Words from 2nd list which didn\'t match the first list.)  Running keywordparty will always overwrite the output files\r\nYou can use: python keywordparty.py yourlist NUL↵ to remove duplicates only without compare or to generate a list of all unique text from any text or binary file.', usage='python keywordparty.py [-h] | [-n] list1_filename list2_filename',
    formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("list1_filename", type=str, help="1st List [input file1]")
parser.add_argument("list2_filename", type=str, help="2nd List [input file2]")
parser.add_argument("-n", action="store_true", help="[-n] Use 'Newline' instead of 'Space' Word Separator.  Only affects OUTPUT files because keywordparty will accept input files with words separated by either newline(s) or space(s) and tab(s).  You can use binary file input to strip words and $pec^a\\ Ch@racter5.")
args = parser.parse_args()
''' Load 1st List to Compare '''
with open(args.list1_filename, "br") as file1:
    file1_bytes = file1.read()
wword1 = []
list1 = []
for c in file1_bytes:
    if c > 8 and c < 12 or c > 31 and c < 127:
        if c != 9 and c != 10 and c != 11 and c != 32:
            wword1.append(str(chr(c)))
        if c == 9 or c == 10 or c == 11 or c == 32:
            fwword1 = "".join(wword1)
            if fwword1:
                list1.append(fwword1)
            wword1 = []
''' Remove Duplicates from 1st List '''
list11 = []
for v in list1:
    if v not in list11:
        list11.append(v)
''' Load 2nd List to Compare '''
with open(args.list2_filename, "br") as file2:
    file2_bytes = file2.read()
wword2 = []
list2 = []
for e in file2_bytes:
    if e > 8 and e < 12 or e > 31 and e < 127:
        if e != 9 and e != 10 and e != 11 and e != 32:
            wword2.append(str(chr(e)))
        if e == 9 or e == 10 or e == 11 or e == 32:
            fwword2 = "".join(wword2)
            if fwword2:
                list2.append(fwword2)
            wword2 = []
''' Remove Duplicates from 2nd List '''
list22 = []
for x in list2:
    if x not in list22:
        list22.append(x)
#Build Matched List with Inverse Lists (Output common_match, leftover1, leftover2 list files
common = []
for i, g in enumerate(list11):
    for l, k in enumerate(list22):
        if g == k:
            common.append(g)
            list11[i] = ""
            list22[l] = ""
leftover1 = []
leftover2 = []
for m in list22:
    if m:
        leftover2.append(m)
for n in list11:
    if n:
        leftover1.append(n)
#add newline to ends of records
if leftover2:
    leftover2[-1] = leftover2[-1] + "\r\n"
if leftover1:
    leftover1[-1] = leftover1[-1] + "\r\n"
if common:
    common[-1] = common[-1] + "\r\n"
if args.n == False:
    foleftover1 = " ".join(leftover1).encode()
    focommon = " ".join(common).encode()
    foleftover2 = " ".join(leftover2).encode()
if args.n == True:
    foleftover1 = "\r\n".join(leftover1).encode()
    focommon = "\r\n".join(common).encode()
    foleftover2 = "\r\n".join(leftover2).encode()

''' write files to disc '''
try:
    with open("common_match", "wb") as fmatch:
        fmatch.write(focommon)
        s = len(focommon)
    print(f"common_match written successfully.  {s:,} bytes")
except IOError as p:
    print(f"Error writing to file: {p}")
try:
    with open("leftover1", "wb") as fl1:
        fl1.write(foleftover1)
        t = len(foleftover1)
    print(f"leftover1 written successfully.  {t:,} bytes")
except IOError as q:
    print(f"Error writing to file: {q}")
try:
    with open("leftover2", "wb") as fl2:
        fl2.write(foleftover2)
        u = len(foleftover2)
    print(f"leftover2 written successfully.  {u:,} bytes")
except IOError as r:
    print(f"Error writing to file: {r}")
